...
export async function loadCanaisOrigem() {
  try {
    const { data } = await _ctx.sb.from('canais_origem')
      .select('*')
...
async function _executeAtendimento(vendedorId, canalOrigemId) {
  ...
  const fila = _ctx.vendedores.filter(x => (x.setor || 'loja') === setor && x.status === 'disponivel' && x.posicao_fila != null).sort((a, b) => a.posicao_fila - b.posicao_fila);
  ...
  try {
    ...
    const { data: atend } = await _ctx.sb.from('atendimentos').select('*, vendedores(nome, apelido), canais_origem(nome, icone)').eq('id', data).single();
...
async function handleProximoCliente() {
  ...
  try {
    const { data, error } = await _ctx.sb.rpc('proximo_cliente', { p_turno_id: _ctx.currentTurno.id, p_setor: _ctx.currentSetor });
    if (error) throw error;
    const { data: atend } = await _ctx.sb.from('atendimentos').select('*, vendedores(nome, apelido), canais_origem(nome, icone)').eq('id', data).single();
    _ctx.activeAtendimentos.push(atend);
    renderActiveAtendimentos();
...
export function renderActiveAtendimentos() {
  ...
  _timerInterval = setInterval(() => {
    ...
    if (allDetached && _timerRefs.size > 0) { clearInterval(_timerInterval); _timerInterval = null; }
  }, 1000);
...
async function finalizeMultiOutcome() {
  ...
  try {
    ...
    for (let i = 1; i < _multiResults.length; i++) {
      ...
      const { data: novoId } = await _ctx.sb.from('atendimentos').insert({
...
export async function checkActiveAtendimentos() {
  if (!_ctx.currentTurno) return;
  const { data } = await _ctx.sb.from('atendimentos').select('*, vendedores(nome, apelido), canais_origem(nome, icone)').eq('turno_id', _ctx.currentTurno.id).eq('resultado', 'em_andamento').order('inicio');
