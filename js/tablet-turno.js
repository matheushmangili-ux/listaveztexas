...
async function openTurnoSummary() {
  ...
  try {
    ...
    const { data: stats } = await _ctx.sb.rpc('get_conversion_stats', {
      p_inicio: turnoStart,
    ...
    // Tempo médio
    const { data: atends } = await _ctx.sb
      .from('atendimentos')
      .select('inicio, fim')
    ...
    // Top vendedor
    const { data: ranking } = await _ctx.sb
      .from('atendimentos')
    ...
    if (ranking && ranking.length > 0) {
      ...
      ranking.forEach((r) => {
        ...
        if (!countMap.has(vid))
          countMap.set(vid, { nome: r.vendedores?.apelido || r.vendedores?.nome || '?', vendas: 0 });
    ...
    const { data: trocasData } = await _ctx.sb
      .from('atendimentos')
      .select('valor_venda')
      .eq('turno_id', _ctx.currentTurno.id)
      .eq('resultado', 'troca');
    ...
    content.innerHTML = `
      ...
      ${
        inQueueCount > 0
          ? `<div style="background:rgba(251,146,60,.08);border:1px solid rgba(251,146,60,.2);border-radius:10px;padding:10px;text-align:center;font-size:13px;color:var(--warning);font-weight:600">
    ...
    content.innerHTML =
      '<div style="text-align:center;color:var(--text-muted);font-size:13px">Erro ao carregar resumo</div>';
  }
}
...
async function confirmCloseTurno() {
  ...
  const resetBtn = () => {
    if (btn) {
      btn.disabled = false;
      btn.innerHTML = '<i class="fa-solid fa-power-off" style="margin-right:4px"></i>Encerrar Turno';
    }
  };
  if (btn) {
    btn.disabled = true;
...
async function confirmCheckin() {
  ...
  if (checked.length === 0) {
    toast('Selecione pelo menos um vendedor', 'warning');
    return;
  }
  ...
  try {
    const { data, error } = await _ctx.sb
      .from('turnos')
      .insert({ data: new Date().toISOString().split('T')[0], tenant_id: _ctx.tenantId })
      .select()
      .single();
    if (error) {
      toast('Erro ao abrir turno', 'error');
      resetBtn();
      return;
    }
    ...
    if (errFila) {
      toast('Erro ao montar fila', 'error');
      resetBtn();
      return;
    }
