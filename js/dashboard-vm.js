...
(function () {
  ...
  const CATS = [
    { id: 'vitrine', label: 'Vitrine' }, { id: 'gondola', label: 'Gôndola' },
    { id: 'display', label: 'Display' }, { id: 'prateleira', label: 'Prateleira' },
    { id: 'checkout', label: 'Checkout' }, { id: 'fachada', label: 'Fachada' },
  ...
  async function renderModal() {
    ...
    body.querySelectorAll('.vm-admin-tab').forEach(btn => {
      btn.addEventListener('click', () => { _activeTab = btn.dataset.t; _viewingTaskId = null; renderModal(); });
    });
  ...
  function renderCreateForm() {
    ...
    // Reference photos
    document.getElementById('vmRefAddBtn')?.addEventListener('click', () => document.getElementById('vmRefInput')?.click());
    ...
    function renderRefPreviews() {
      ...
      if (!wrap) return;
      wrap.innerHTML = refFiles.map((f, i) => {
      ...
      }).join('') + (refFiles.length < 5 ? '<button class="vm-ref-add" id="vmRefAddBtn2"><i class="fa-solid fa-plus"></i></button>' : '');
      wrap.querySelectorAll('.vm-ref-remove').forEach(btn => {
        btn.addEventListener('click', () => { refFiles.splice(parseInt(btn.dataset.idx), 1); renderRefPreviews(); });
      });
      document.getElementById('vmRefAddBtn2')?.addEventListener('click', () => document.getElementById('vmRefInput')?.click());
    }
    ...
    function renderChecklist() {
      ...
      if (!wrap) return;
      wrap.innerHTML = checkItems.map((label, i) =>
      ...
      wrap.querySelectorAll('.vm-checklist-input').forEach(input => {
        input.addEventListener('input', () => { checkItems[parseInt(input.dataset.idx)] = input.value; });
      });
      wrap.querySelectorAll('.vm-checklist-remove').forEach(btn => {
        btn.addEventListener('click', () => { checkItems.splice(parseInt(btn.dataset.idx), 1); renderChecklist(); });
      });
    }
    ...
    document.getElementById('vmCreateBtn')?.addEventListener('click', async () => {
      ...
      if (!title || !selectedCat) { window._toast?.('Preencha título e categoria', 'error'); return; }
      ...
      try {
        ...
        if (btn) { btn.disabled = false; btn.textContent = 'Criar e enviar'; }
      }
    });
  }
  ...
  async function renderTrackView() {
    ...
    if (_viewingTaskId) { await renderTaskDetail(area); return; }
    ...
    if (_tasks.length === 0) {
      area.innerHTML = '<div style="padding:24px;text-align:center;color:var(--text-muted);font-style:italic">Nenhuma tarefa ativa. Crie uma na aba "Criar Tarefa".</div>';
      return;
    }

    area.innerHTML = _tasks.map(t => {
    ...
    area.querySelectorAll('.vm-track-card').forEach(card => {
      card.addEventListener('click', () => { _viewingTaskId = card.dataset.tid; renderTrackView(); });
    });
  }
  ...
  async function renderTaskDetail(area) {
    ...
    area.innerHTML = backBtn + `<h4 style="margin:8px 0 12px">${esc(task?.title || '')}</h4>` +
      (_taskSubs.length === 0 ? '<div style="padding:12px;color:var(--text-muted);font-style:italic">Nenhuma submissão ainda.</div>' :
      '<div class="vm-admin-list">' + _taskSubs.map(s => {
        const nome = s.vendor_apelido || s.vendor_nome || '—';
        const photos = s.photos || [];
        const checks = s.checklist_responses || [];
        const statusLabel = { pending: 'Pendente', in_progress: 'Em execução', submitted: 'Enviado', approved: 'Aprovado', rejected: 'Rejeitado', revision: 'Ajuste' }[s.status] || s.status;
        ...
        return `<div class="vm-admin-item" data-aid="${s.assignment_id}">
          <div class="vm-admin-info" style="width:100%">
            ...
            ${showActions ? `<div class="vm-admin-actions">
    ...
    document.getElementById('vmBackBtn')?.addEventListener('click', () => { _viewingTaskId = null; renderTrackView(); });
  ...
  async function reviewTask(assignmentId, status, feedback) {
    try {
      const { error } = await sb.rpc('admin_review_task_submission', {
        p_assignment_id: assignmentId, p_status: status, p_feedback: feedback || null
      });
      if (error) throw error;
  ...
  async function renderFreeView() {
    ...
    const list = _queue.length === 0
      ...
      : '<div class="vm-admin-list">' + _queue.map(item => {
        ...
        const actions = _freeStatus === 'pending' ? `
    ...
    area.querySelectorAll('[data-fs]').forEach(btn => {
      btn.addEventListener('click', () => { _freeStatus = btn.dataset.fs; renderFreeView(); });
    });
    area.querySelectorAll('.vm-free-approve').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        } catch (err) { window._toast?.(err?.message, 'error'); }
      });
    });
    area.querySelectorAll('.vm-free-reject').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        try {
          const { error } = await sb.rpc('admin_review_vm', { p_submission_id: btn.dataset.id, p_status: 'rejected', p_feedback: feedback });
          if (error) throw error;
  ...
  function resizeImg(file, maxSize, quality) {
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        c.toBlob(b => { URL.revokeObjectURL(u); b ? resolve(b) : reject(new Error('toBlob failed')); }, 'image/jpeg', quality);
      };
      img.onerror = () => { URL.revokeObjectURL(u); reject(new Error('img load failed')); };
      img.src = u;
    });
  }
